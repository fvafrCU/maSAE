...
## where 'equationnumber. ist the equatoin number from the manuscript
...
setClass(Class = "saData"
	 , slots = c(
					   data = "data.frame"
					   , predictand = "character"
					   , smallArea = "character"
					   , cluster = "characterOrNULL"
					   , include = "characterOrNULL"
					   )
	 , validity = function(object){
	   if(! is.numeric(object@data[, object@predictand]))
	     return(paste("data$", object@predictand, " has got to be numeric.", sep=''))
	   if(! is.null(object@cluster))
	     if(! object@cluster %in% colnames(object@data))
	       return(paste("data$", object@cluster, " not found.", sep=''))
	   if(! is.null(object@include)){
	     if(! object@include %in% colnames(object@data))
	       return(paste("data$", object@include, " not found.", sep=''))
	     if(class(object@data[, object@include]) != "logical")
	       return(paste(object@include, " has got to be of class 'logical'.", sep=''))
	   }
	 }
	 )
	     saData <- function(data, predictand
				 , smallArea
	     ...
	     setMethod(f = "predict"
			 ...
			 include <- object@data[, object@include]
			 if (! is.null(object@cluster))
			 {# cluster sampling 
			     M <- tapply(as.numeric(include), object@data[, object@cluster], sum, na.rm = TRUE)
			     predictand_c <- tapply(object@data[, object@predictand] * as.numeric(include)
						    , object@data[, object@cluster], sum) / M
			 } 
			 else 
			 {# non-cluster sampling
			   predictand <- object@data[include, object@predictand]
			 }
			 out <- data.frame()
			 for (group in sort(unique(object@data[, object@smallArea])[! is.na(unique(object@data[, object@smallArea]))]))
			 {
			     
			   if (! is.null(object@cluster))
			   {# cluster sampling
			     if (is.factor(object@data[, object@smallArea])){
			       smallarea <- tapply(as.character(object@data[, object@smallArea]), object@data[, object@cluster], unique)
			     } else {
			       smallarea <- tapply(object@data[, object@smallArea], object@data[, object@cluster], unique)
			     }
			     ...
			     var_cs <- 1 / sum(M[Gc])^2 * n_Gc / (n_Gc - 1) * sum((predictand_c[Gc] * M[Gc] 
										   - p_cs * M[Gc])^2) 
			   ...
			   {# non-cluster sampling 
			       G <- object@data[, object@smallArea] == group; G[is.na(G)] <- FALSE
			       result <- data.frame(smallArea = group
						    , p_srs  = mean(predictand[G])
						    , var_srs = var(predictand[G]) / length(predictand[G])
						    )
			   }
...
setClass(Class = "saeData"
	 ...
	 , validity = function(object){
	   if(! is.numeric(as.matrix(object@data[, c(object@predictors)])))
	     return(paste("data$", object@predictors, " has got to be numeric.", sep=''))
	   if(! is.null(object@s1)){
	     if(class(object@data[, object@s1]) != "logical")
	       return(paste("data$", object@s1, " has got to be of class 'logical'.", sep=''))
	     s1TOs0 <- sum(object@data[,object@s1]) /  nrow(object@data)
	     if( s1TOs0 > 0.1)
	       message(paste("n(s0) >> n(s1) should hold, but you've given s1 resulting in n(s1)/n(s0) = ",s1TOs0))
	   }
	   if(! is.null(object@s2)){
	     if(class(object@data[, object@s2]) != "logical")
	       return(paste("data$", object@s2, " has got to be of class 'logical'.", sep=''))
	   } else {
	     if(!is.null(object@cluster))
	       return('need s2 for cluster sampling.')
	     if(is.null(object@smallAreaMeans))
	       return("got neither s2 nor smallAreaMeans.")
	     else {
	       if(! all(object@predictors %in% colnames(object@smallAreaMeans) ))
		 return("got neither s2 nor exhaustive smallAreaMeans.")
	     }
	   }
	   if(! is.null(object@s1) && ! is.null(object@s2))
	     if(any(object@data[,object@s2] &  ! object@data[,object@s1]))
	       return('s2 is not a subset of s1!')
	   if(! is.null(object@smallAreaMeans)){
	     if(! all(colnames(object@smallAreaMeans) %in% c(object@predictors, object@smallArea)))
	       return(paste("smallAreaMeans don't really fit. 
			    ...
			    if(any(is.na(object@smallAreaMeans)))
			      return("Can't deal with missing data in smallAreaMeans")
			    if(!all(object@smallAreaMeans[,object@smallArea] %in%
				    unique(object@data[,object@smallArea][!is.na(object@data[,object@smallArea])])))
			      return('found extraneous smallAreas in smallAreaMeans')
			    if(!all(unique(object@data[,object@smallArea][!is.na(object@data[,object@smallArea])]) %in%
				    object@smallAreaMeans[,object@smallArea]))
			      return('missing smallAreas in smallAreaMeans')
	   }
	 }
	 )
	     saeData <- function(data, predictand, predictors
				 , smallArea, smallAreaMeans = NULL
				 , s1 = NULL, s2 = NULL
				 , cluster = NULL, include = NULL)
	     {
	       com <- NULL
	     ...
	     setGeneric(name = "msae"
			, def = function(object, ...){standardGeneric("msae")}
			)
	     setMethod(f = "predict"
		       , signature(object = "saeData")
		       , function(object
				  , ALL = FALSE
				  , design = FALSE
				  ){
			 ...
			 if (VIMFOLD){ ## which type of auxiliary data do we have?
			   if (is.null(object@smallAreaMeans)){
			     ...
			     if(
				length(colnames(object@smallAreaMeans)) == length(c(object@predictors, object@smallArea)) && 
				all(sort(colnames(object@smallAreaMeans)) == sort(c(object@predictors, object@smallArea)))
				){
			 ...
			 if (VIMFOLD){ ## cluster and non-cluster commons
			   if (pred.type == "partially")
			   {
			     if (threePhase){
			       ...
			       predictors1 <- object@predictors[
								apply(
								      ! is.na(
									      object@data[! object@data[, object@s1], object@predictors]
									      )
								      ,2
								      ,all)]

			     } else {
			       predictors1 <- colnames(object@smallAreaMeans)[-which(colnames(object@smallAreaMeans) == object@smallArea)]
			     }
			     Z1 <-  as.list(as.data.frame(t(cbind(1, object@data[, predictors1]))))  
			   }
			   if (is.null(object@cluster) || 
			       pred.type == "partially")
			   {
			     if (is.null(object@s2)){ 
			       s2 <- rep(TRUE, nrow(object@data)) 
			     }  else { 
			       s2 <- object@data[, object@s2]
			     }
			     n_s2 <- sum(as.numeric(s2))
			     predictand <- object@data[, object@predictand]
			     Z <- as.list(as.data.frame(t(cbind(1, object@data[, c(object@predictors)]))))
			   }
			 }
			 if (! is.null(object@cluster))
			 {# cluster sampling 
			   if (VIMFOLD){ ## commons
			     if (is.null(object@s1)){ 
			       ...
			       s1c <- rep(TRUE, length(unique(object@data[, object@cluster]))) 
			       names(s1c) <- unique(object@data[, object@cluster])
			     } else {
			       s1c <-   tapply(object@data[, object@s1], object@data[, object@cluster], unique)
			     }
			     s2c <-   tapply(object@data[, object@s2], object@data[, object@cluster], unique)
			     ...
			     Z_c <- mapply(append
					   , mapply('/'
						    , by( object@data[, c(object@predictors)] *  as.numeric(include)
							 , object@data[, object@cluster], sum, simplify = TRUE)
						    , M
						    , SIMPLIFY=FALSE)
					   , 1
					   , SIMPLIFY = FALSE
			     ...
			     Z_c <- mapply(append
					   , mapply('/'
						    , by( object@data[, c(object@predictors)] *  as.numeric(include)
							 , object@data[, object@cluster], colSums, simplify = TRUE)
						    , M
						    , SIMPLIFY=FALSE)
					   , 1
					   , SIMPLIFY = FALSE
			     ...
			     a2.38 <- as.vector(iA_cs2c %*% Reduce('+', mapply('*', M[s2c]
									       ,   mapply('*', predictand_c[s2c]
											  , Z_c[s2c]
											  , SIMPLIFY=FALSE)
									       , SIMPLIFY=FALSE)
			     ) / n_s2c)
			     ## calculate residuals 
			     R_c <- predictand_c-sapply(Z_c, function(x) x %*% a2.38) ## a2 p.445
			 ...
			 {# non-cluster sampling
			   if (VIMFOLD){ ## commons
			     if (is.null(object@s1)){
			       s1 <- rep(TRUE, nrow(object@data))
			     } else {
			       s1 <- object@data[, object@s1]
			     }
			     ## set up A_s2
			     A_s2 <- Reduce('+', lapply(Z[s2], function(x) as.numeric(x) %o% as.numeric(x)))/ n_s2
			     iA_s2 <- solve(A_s2)
			     ## estimate regression coefficients
			     u_s2 <- Reduce('+', mapply('*', Z[s2], predictand[s2], SIMPLIFY = FALSE)) /  n_s2
			     a2.13.b <- as.numeric(iA_s2 %*% u_s2)
			     ## calculate residuals 
			     R <- predictand - sapply(Z, function(x) x %*% a2.13.b) # a2 p.443 & b2 p.1025
			     ## design-based variance-covariance matrix
			   ...
			   switch(pred.type # uncommons
				  ...
				  , 'partially' = {
				    ...
				    predictors2 <- object@predictors[-which(object@predictors %in%  predictors1)]
				    A1_s2 <- Reduce('+', lapply(Z1[s2], function(x) as.numeric(x) %o% as.numeric(x)))/ n_s2 ## from b2.27
				    iA1_s2 <- solve(A1_s2)
				    u1_s2 <- Reduce('+', mapply('*', Z1[s2], predictand[s2], SIMPLIFY = FALSE)) /  n_s2
				    b2.5.2 <- as.numeric(iA1_s2 %*% u1_s2)
				    R1 <- predictand - sapply(Z1, function(x) x %*% b2.5.2)  # b2 p.1025
				    b2.6.2 <- a2.13.b
				    b1.28 <- a2.15 
			 ...
			 for (group in sort(unique(object@data[, object@smallArea])[! is.na(unique(object@data[, object@smallArea]))]))
			 {
			   if (VIMFOLD){ # cluster and non-cluster commons, temporary objects will be removed for cluster sampling
				 ...
				 pred.type == "partially")
			     {
			       ...
			       eu_s2 <- Reduce('+', mapply('*', eZ[s2], predictand[s2], SIMPLIFY = FALSE)) /  n_s2  ## a2 p.444
			     ...
			     switch(pred.type 
				    , 'partially' = {
				      ...
				      eu1_s2 <- Reduce('+', mapply('*', eZ1[s2], predictand[s2], SIMPLIFY = FALSE)) /  n_s2 
				      b2.27 <- ieA1_s2 %*% eu1_s2 ## needed in cluster sampling, see b1 p.22
				      tmZ1_G <- as.numeric(
							   cbind(1
								 , subset(object@smallAreaMeans[, predictors1]
									  , object@smallAreaMeans[, object@smallArea] == group))
							   ) ## b2 p.1026
			   ...
			   {# cluster sampling
			     ...
			     var_cs <- 1 / sum(M[s2c & Gc])^2 * n_s2cGc / (n_s2cGc - 1) * sum((predictand_c[s2c & Gc] * M[s2c & Gc] 
											       - p_cs * M[s2c & Gc])^2) 
			     result <- data.frame(smallArea = group
						  , p_cs = p_cs
						  , var_cs = var_cs
			     ...
			     if (VIMFOLD){ # commons
			       ## extended model
			       tmp <- object@data[, object@smallArea] == group; tmp[is.na(tmp)] <- FALSE
			       I_cGc <- tapply(as.numeric(tmp), object@data[, object@cluster], sum) / M ## a2 p.445: I_{c, Gc}(x) !:= Gc
			       ...
			       a1.63 <- ieA_cs2c %*% Reduce('+', mapply('*', eZ_c[s2c]
									, predictand_c[s2c] * M[s2c]
									, SIMPLIFY = FALSE)) / n_s2c
			       eR_c <- predictand_c - sapply(eZ_c, function(x) x %*% a1.63) # a1 p.25
			     ...
			     switch(pred.type # uncommons
					...
					Z1_c <- mapply(append
						       , mapply('/'
								, tapply( object@data[, predictors1] *  as.numeric(include)
									 , object@data[, object@cluster], sum)
								, M
								, SIMPLIFY=FALSE)
						       , 1
						       , SIMPLIFY = FALSE
					...
					Z1_c <- mapply(append
						       , mapply('/'
								,
								by( object@data[, predictors1] *  as.numeric(include)
								   , object@data[, object@cluster], colSums, simplify = TRUE)
								, M
								, SIMPLIFY=FALSE)
						       , 1
				      ...
				      com <- c(com,w)
				      eR1_c <- predictand_c - sapply(eZ1_c, function(x) x %*% b1.48) # b1 p.22
				      ...
				      eR_c <- predictand_c - sapply(eZ_c, function(x) x %*% b1.49) # b1 p.22
				    ...
				    , 'exhaustive' = {
				      tmZ_Gc <- as.numeric(
							   cbind(1
								 , subset(object@smallAreaMeans[, object@predictors]
									  , object@smallAreaMeans[, object@smallArea] == group))
				    ...
				    , 'non-exhaustive' = {
				      ## ## ## psynth
				      ## mean of the clustered predictand m over Gc
			   ...
			   {# non-cluster sampling 
			       ...
			       if (design){
			       result <- data.frame(smallArea = group
						    , p_srs  = mean(predictand[s2 & G])
						    , var_srs = var(predictand[s2 & G]) / n_s2G
						    )
			       } else {
				 result <- data.frame(smallArea = group)
			       }
			     if (VIMFOLD){ # commons
			       ...
			       ## extended model
			       eR <- predictand - sapply(eZ, function(x) x %*% b2.28)  ## a2 p.444
			     ...
			     switch(pred.type # commons to non-exhaustive & partially 
				    ...
				    , 'partially' = {
				      if (length(object@predictors) == 1){
					mZ_s1G <- c(1, mean(object@data[, c(object@predictors)][s1 & G])) ## a2 p.443, b1 p.15
				      } else {
					mZ_s1G <- c(1, colMeans(object@data[, c(object@predictors)][s1 & G, ])) ## a2 p.443, b1 p.15
				      }
			     ...
			     switch(pred.type # uncommons 
					...
					mZ1_s1G <- as.vector(mZ_s1G[-which(names(mZ_s1G) == predictors2)])
				      ...
				      b2.29 <- a2.30
				      eR1 <- predictand - sapply(eZ1, function(x) x %*% b2.27) # b1 p.18
				      ...
				      if(threePhase){ # we now estimate the smallAreaMeans from s0: c1 p.16. This is the essence of c1.
					if (length(predictors1) == 1){
					  tmeZ1_G  <- c(1, mean(object@data[G, predictors1]),  1) 
					} else {
					  tmeZ1_G  <- c(1, colMeans(object@data[G, predictors1]),  1) 
					}
				      }
